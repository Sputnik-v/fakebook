...
@section('content')
    <aside class="flex flex-col w-64 h-screen px-4 py-8 overflow-y-auto bg-white border-r rtl:border-r-0 rtl:border-l dark:bg-gray-900 dark:border-gray-700">
        <p class="mx-auto font-bold text-gray-400">Personal Area</p>

        <div class="flex flex-col items-center mt-6 -mx-2">
            <form method="POST"  enctype="multipart/form-data">
                <label for="avatar" class="bg-gray-500 w-full h-4">
                    <input class="hidden" type="file" name="avatar" id="avatar">
                </label>
            </form>
            <img class="object-cover w-24 h-24 mx-2 rounded-full" @if($srcImage) src="{{$srcImage['image']}}" @else src="{{asset('images/user.png')}}" @endif  alt="avatar">
            <h4 class="mx-2 mt-2 font-medium text-gray-800 dark:text-gray-200">{{$name}}</h4>
            <p class="mx-2 mt-1 text-sm font-medium text-gray-600 dark:text-gray-400">{{$email}}</p>
        </div>

{{--        image download--}}

        <div x-data="{ isOpen: false }" class="relative flex justify-center mt-2">
                ...
                <div class="flex items-end justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
                    ...
                    <div>
                        <form method="POST" action="{{route('image.store')}}" enctype="multipart/form-data">
                            @csrf
                            <div>
                                <label for="image" class="block text-sm text-center text-gray-500 dark:text-gray-300">Image</label>
                            ...
                            <div class="sm:flex flex justify-end sm:items-center mt-2">
                        ...
                        <button @click="isOpen = false" class="w-full px-4 py-2 mt-2 text-sm font-medium tracking-wide text-gray-700 capitalize transition-colors duration-300 transform border border-gray-200 rounded-md sm:mt-0 sm:w-auto sm:mx-2 dark:text-gray-200 dark:border-gray-700 dark:hover:bg-gray-800 hover:bg-gray-100 focus:outline-none focus:ring focus:ring-gray-300 focus:ring-opacity-40 absolute right-0 top-0">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
